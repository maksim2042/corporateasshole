...
#print `getCompaniesBypostalCode(token,'20024')`

def findcompany(company,auth_token=token,version=4.0):
    url = 'https://maxcvservices.dnb.com/V'+str(version)+'/organizations'
    r = requests.get(url,headers={'Authorization':auth_token},params={'OrganizationName':company,'findcompany':True,
    "TradeUp":True,"CleanseMatch":True})
    assert r.status_code==200,"Invalid duns or product_id"
